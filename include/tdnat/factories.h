...
template <typename T> struct Scalar {
  static std::string name() { return "Scalar<" + LLVMType<T>::name() + ">"; }
  static at::Scalar create(T val) { return {val}; }
};

template <typename T> struct ArrayRef {
  static std::string name() { return "ArrayRef<" + LLVMType<T>::name() + ">"; }
  static at::ArrayRef<T> create(T *list, size_t size) { return {list, size}; }
};

template <typename T> struct Optional {
  static std::string name() { return "Optional<" + LLVMType<T>::name() + ">"; }
...
struct OptionalTensorList {
  static std::string name() { return "List<Optional<Tensor>>"; }

  static c10::List<c10::optional<at::Tensor>>
  create(c10::optional<at::Tensor> *list, size_t size) {
