...
def alignedargs_into_values(aligned_arguments: Sequence[nat.AlignedArg], input_args: Dict[int, nat.AlignedArg], fn: nat.Function) -> List[nat.Value]:
    placeholder_idx = 0
    arg_values = []
        ...
        if i in input_args:
            if isinstance(param.type, ListType):
                ...
                for i, tensor in enumerate(arg.value):
                    list_values.append(into_value(param.type.elem, f"{param.name}_{i}", tensor, placeholder_idx, fn))
                ...
                arg_values.append(into_value(param.type, param.name, arg.value, placeholder_idx, fn))
                placeholder_idx += 1
        else:
...
def build_function_for_native(f: NativeFunction, op_name: str, sample: SampleInput, id: str, out_tensors: int) -> Callable[[], List[torch.Tensor]]:
    aligned_args = nat.align_and_flat_arguments(f.func.arguments.flat_all, [sample.input, *sample.args], sample.kwargs)
...
def equals(l: torch.Tensor, r: torch.Tensor, dtype: torch.dtype) -> bool:
    if dtype == torch.int:
        return torch.equal(l, r)
    return bool(torch.isclose(l.to_dense(), r.to_dense(), equal_nan=True).all())
...
class TestOps(unittest.TestCase):
    ...
    def test_jit(self, device, dtype, op: OpInfo):
        ...
        for sample in samples:
            try:
                op_overloaded_name = nat.find_operator_name(op.name, args=[sample.input, *sample.args], kwargs=sample.kwargs)
            ...
            try:
                expected: Union[torch.Tensor, List[torch.Tensor]] = op(sample.input, *sample.args, **sample.kwargs)
            except Exception:
            ...
            result = build_function_for_native(nativef, op_overloaded_name, sample, function_id, out_tensors)()
