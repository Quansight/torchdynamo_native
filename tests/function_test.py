...
class SpecSum(Spec):
    ...
    def jit(self) -> List[torch.Tensor]:
        ...
        dim_values = [fn.build_integer(d) for d in self.dim]
...
class SpecChunk(Spec):
    ...
    def jit(self) -> List[torch.Tensor]:
        ...
        chunks = fn.build_integer(self.chunks);
        dim = fn.build_integer(self.dim);

        chunk = fn.add_call("chunk", "chunk", [tensor, chunks, dim]);

        values = [
            fn.build_vector_at_tensor(chunk, fn.build_integer(i))
